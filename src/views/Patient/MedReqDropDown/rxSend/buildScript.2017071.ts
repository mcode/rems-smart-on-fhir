...
import { Address, FhirResource, HumanName, MedicationRequest, MedicationRequestDispenseRequest, Patient, Practitioner } from "fhir/r4";

function xmlAddTextNode(xmlDoc: XMLDocument, parent: HTMLElement, sectionName: string, value: string) {
...
function buildNewRxAddress(doc: XMLDocument, addressResource: Address) {
  ...
  addressResource.postalCode && xmlAddTextNode(doc, address, "PostalCode", addressResource.postalCode);
...
function buildNewRxPatient(doc: XMLDocument, patientResource: Patient) {
  ...
  patientAddressResource && humanPatient.appendChild(buildNewRxAddress(doc, patientAddressResource));
...
function buildNewRxPrescriber(doc: XMLDocument, practitionerResource: Practitioner) {
  ...
  if (practitionerResource.identifier && practitionerResource.telecom) {
    ...
    practitionerNameResource && nonVeterinarian.appendChild(buildNewRxName(doc, practitionerNameResource));

    //     Prescriber Address
    const practitionerAddressResource = practitionerResource.address?.[0];
    practitionerAddressResource && nonVeterinarian.appendChild(buildNewRxAddress(doc, practitionerAddressResource));
...
function quantityUnitOfMeasureFromDrugFormCode(dispenseRequest: MedicationRequestDispenseRequest) {
  ...
  if (dispenseRequest.quantity?.system?.toLowerCase().endsWith("v3-orderableDrugForm".toLowerCase()) && dispenseRequest.quantity.code) {
  ...
  if (medicationCodingList) {
    for (let i = 0; i < medicationCodingList.length; i++) {
      ...
      if (system && system.endsWith("rxnorm")) {
        ...
        coding.display && xmlAddTextNode(doc, medicationPrescribed, "DrugDescription", coding.display);
    ...
    dispenseRequest?.quantity?.value && xmlAddTextNode(doc, quantity, "Value", dispenseRequest.quantity.value.toString());
    ...
    dispenseRequest && xmlAddTextNode(doc, quantityUnitOfMeasure, "Code", quantityUnitOfMeasureFromDrugFormCode(dispenseRequest));
    ...
    medicationRequestResource.authoredOn && xmlAddTextNode(doc, writtenDate, "Date", medicationRequestResource.authoredOn);
    ...
    dispenseRequest?.numberOfRepeatsAllowed && xmlAddTextNode(doc, medicationPrescribed, "NumberOfRefills", dispenseRequest.numberOfRepeatsAllowed.toString());

    //     Medication Sig
    var sig = doc.createElement("Sig");
    medicationRequestResource.dosageInstruction?.[0].text  && xmlAddTextNode(doc, sig, "SigText", medicationRequestResource.dosageInstruction?.[0].text);
...
export default function buildNewRxRequest(patientResource: Patient, practitionerResource: Practitioner, medicationRequestResource: MedicationRequest) {
  // console.log(medicationRequestResource);
