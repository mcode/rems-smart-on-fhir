...
export default function RemsInterface(props: RemsInterfaceProps){
  ...
  const sendGetRx = () => {
    if (props.specialtyRxBundle.entry && props.specialtyRxBundle.entry[0].resource) {
      ...
      const messageHeader: MessageHeader = props.specialtyRxBundle.entry[0].resource as MessageHeader;
      if(messageHeader.focus?.[0]?.reference){
        let potentialParams = getResource(props.specialtyRxBundle, messageHeader.focus[0].reference);
        if(potentialParams){
          ...
          if(params.parameter){
            for (let param of params.parameter) {
              if (param.name === "prescription" && param.valueReference && param.valueReference.reference) {
                prescriptionReference = param.valueReference.reference;
              }
              else if (param.name === "source-patient" && param.valueReference && param.valueReference.reference) {
          ...
          if(potentialPrescription && potentialPatient) {
            ...
            const simpleDrugName = prescription.medicationCodeableConcept?.coding?.[0].display?.split(" ")[0];
            let rxDate = prescription.authoredOn;
            ...
            axios.get(`http://localhost:5051/doctorOrders/api/getRx/${patientFirstName}/${patientLastName}/${patientDOB}?simpleDrugName=${simpleDrugName}&rxDate=${rxDate}`, getAxiosOptions()).then((response) => {
  ...
  const refreshBundle = () => {
    setSpin(true)
    axios.get(`http://localhost:8090/etasu/met/${remsAdminResponse?.data.case_number}`).then((response) => {
  ...
  let hasRemsCase = remsAdminResponse?.data?.case_number ? true : false;
          ...
          <div>
            <div className="container left-form">
              ...
              <Paper style={{ paddingBottom: "5px" }}>
                ...
                <div className="bundle-entry">
                  Status: {remsAdminResponse?.data?.status}
                </div>
                <div className="bundle-entry">
                  <Button variant="contained" onClick={toggleBundle}>View Bundle</Button>
                  <Button variant="contained" onClick={toggleResponse}>View ETASU</Button>
              ...
              {viewResponse ?
                ...
                :
                ""}
              {viewBundle ? <div className="bundle-view">
                <br></br>
                <h3>Bundle</h3>
            ...
            <div className="right-form">
              ...
              {viewPisBundle ? <div className="bundle-view">
                <br></br>
                <h3>Bundle</h3>
                {renderBundle(props.specialtyRxBundle)}
              </div> : ""}
