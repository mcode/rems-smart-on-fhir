...
function fetchArtifactsOperation(
  order: string,
  coverage: string,
  ...
  return new Promise<ReturnValue>(function (resolve, reject) {
    ...
    function handleFetchErrors(response: Response) {
      if (!response.ok) {
        const msg = 'Failure when fetching resource';
          ...
          .then((result: Parameters) => {
            ...
            if (bundle && bundle.resourceType == 'Bundle' && bundle.entry) {
              ...
              if (containedQuestionnaire) {
                ...
                questionnaire = bundleEntries.find(
                  e => e.resource?.resourceType === 'Questionnaire'
                )?.resource as Questionnaire;
                if (questionnaire) {
                  // annoying but type needs to be checked again to satisfy ts
                  retVal.questionnaire = questionnaire;
                }
              }
              retVal.isAdaptiveFormWithoutExtension =
                questionnaire.extension && questionnaire.extension.length > 0;
    ...
    function searchBundle(questionnaire: Questionnaire, bundle: BundleEntry<FhirResource>[]) {
      if (questionnaire.extension !== undefined) {
        ...
        const mainElmReferences = questionnaire.extension
          .filter(ext => ext.url == 'http://hl7.org/fhir/StructureDefinition/cqf-library')
          .map(lib => lib.valueCanonical);
        bundle.forEach(entry => {
          const resource = entry.resource;
          if (resource && resource.resourceType === 'Library' && resource.content) {
            const base64elmData = resource.content.filter(
              c => c.contentType == 'application/elm+json'
            )[0].data;
            // parse the json string
            if (base64elmData) {
              const elm = JSON.parse(Buffer.from(base64elmData, 'base64').toString());
              if (
                mainElmReferences.find(mainElmReference => {
        ...
        if (item.extension) {
          const findEmbeddedCql = item.extension.find(
            ext =>
              ...
              ext.valueExpression &&
              ext.valueExpression.language === 'application/elm+json'
          );

          if (
            findEmbeddedCql &&
            findEmbeddedCql.valueExpression &&
            findEmbeddedCql.valueExpression.expression
          ) {
...
export { fetchArtifactsOperation, fetchFromQuestionnaireResponse, searchByOrder };
