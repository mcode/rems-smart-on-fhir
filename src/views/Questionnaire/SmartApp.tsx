import { Bundle, CodeableConcept, Coding, DeviceRequest, Extension, FhirResource, Library, MedicationDispense, MedicationRequest, Questionnaire, QuestionnaireResponse, ServiceRequest } from 'fhir/r4';
...
import { ReturnValue, fetchArtifactsOperation, fetchFromQuestionnaireResponse, searchByOrder } from './fetchArtifacts';
...
export function SmartApp(props: SmartAppProps) {
    ...
    const [isAdaptiveFormWithoutExtension, setIsAdaptiveFormWithoutExtension] = useState<boolean>(false)
    const [isFetchingArtifacts, setIsFetchingArtifacts] = useState<boolean>(false)
    const [cqlPrepopulationResults, setCqlPrepopulationResults] = useState<PrepopulationResults | null>(null);
    ...
    const standaloneLaunch = (patientId: string, response: QuestionnaireResponse) => {
        fetchFhirVersion(props.smartClient.state.serverUrl)
        .then(fhirVersion => {
    ...
    const ehrLaunch = (isContainedQuestionnaire: boolean, questionnaire: Questionnaire | null) => {
        if(appContext) {
            let acOrder = appContext?.order;
            let acCoverage = appContext?.coverage;
            let acQuestionnaire = appContext?.questionnaire;
            let acResponse = appContext?.response;
            ...
            searchByOrder(acOrder, smart).then((res) => {
                ...
                fetchFromQuestionnaireResponse(acResponse, smart).then((relaunchContext) => {
                    ...
                    if(relaunchContext.order && relaunchContext.coverage && relaunchContext.questionnaire){
                        fetchResourcesAndExecuteCql(relaunchContext.order, relaunchContext.coverage, relaunchContext.questionnaire);
                    }
                });
    ...
    const filter = (defaultFilter: boolean) => {
        ...
        items.map((element) => {
            ...
            if(element.tagName === "INPUT" || element.tagName === "TEXTAREA") { 
              ...
              let inputRowElement = element.closest('.lf-table-item') as HTMLElement;
              if (inputRowElement) {
                if(inputRowElement.classList.contains('lf-layout-horizontal')) {
                  ...
                  let allFilled = true;
                  for(let untypedInput of childrenInputs) {
        ...
        sections.map((element) => {
          if(!element.querySelector(".ng-empty")) {
              ...
              if(actuallyNotEmpty && element.parentElement && !element.parentElement.querySelector(".ng-empty")){
              ...
              if(element.querySelector(".ng-empty:not([disabled]):not(.tooltipContent)")===null && element.parentElement) { 
                  element.parentElement.hidden=checked;
              } else {
    ...
    const fetchResourcesAndExecuteCql = (order: string, coverage: string, questionnaire: string, containedQuestionnaire?: Questionnaire) => {
        fetchFhirVersion(smart.state.serverUrl)
        .then(fhirVersion => {
          FHIR_VERSION = fhirVersion;
    
          fetchArtifactsOperation(order, coverage, questionnaire, smart, consoleLog, containedQuestionnaire)
            .then(artifacts => {
              console.log("fetched needed artifacts:", artifacts);
              const orderResourceArtifact = artifacts.order;
              let fhirWrapper = getFhirWrapper(fhirVersion);
              ...
              if(artifacts.questionnaire) {
                const includedLen = artifacts.questionnaire.extension?.filter((e: Extension) => e.url === "http://hl7.org/fhir/StructureDefinition/cqf-library").length;
                const includedCqf = includedLen ? includedLen > 0 : false
                setIsAdaptiveFormWithoutExtension(artifacts.questionnaire.meta && artifacts.questionnaire.meta.profile && artifacts.questionnaire.meta.profile.includes("http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-adapt") && (artifacts.questionnaire.extension === undefined || !includedCqf)? true: false)
              }
                    ...
                    elmDependencies: mainLibraryElm.library.includes ? mainLibraryElm.library.includes.def.map(
                      includeStatement => {
                        ...
                        if (foundLibrary != null) {
                          ...
                          consoleLog(`Could not find library ${includeStatement.path}. Check if it is referenced in FHIR Library (${mainLibraryElm.library.identifier.id}) properly.`, `errorClass`)
                        }
                      }
                    ) : undefined,
                    valueSetDB: {},
            ...
            .then(cqlResults => {
              ...
              let allLibrariesResults: PrepopulationResults = {};
  ...
  const fillValueSetDB = (executionInputs: ExecutionInputs, artifacts: ReturnValue) => {
    ...
    allElms.forEach(elm => {
      ...
      elm.library.valueSets.def.forEach(valueSetDef => {
        ...
        if (valueSet != null) {
          ...
          consoleLog(`Could not find valueset ${valueSetDef.id}. Try reloading with VSAC credentials in CRD.`, 'errorClass');
        }
      });
    });
  }
    ...
    const onFilterCheckboxRefChange = () => {
        let filterCheckbox = document.getElementById("filterCheckbox") as HTMLInputElement;
    ...
    const renderButtons = (ref: Element) => {
        ...
        <div className="task-button">
            <label>Only Show Unfilled Fields</label>  <input type="checkbox" onChange={()=>{filter(false)}} id="filterCheckbox" ref={onFilterCheckboxRefChange}></input>
        </div></div></div>)
    ...
    if (
        ...
        return isFetchingArtifacts ? (<div> Fetching resources ... </div>) : (
            <div>
                <div className="App">
                    <div
                        className={"overlay " + (showOverlay ? 'on' : 'off')}
                        onClick={() => { console.log(showOverlay); toggleOverlay() }}
                    >

                    </div>
                    {specialtyRxBundle && remsAdminResponse ? (
                        <RemsInterface specialtyRxBundle={specialtyRxBundle} remsAdminResponse={remsAdminResponse} />
                    ) : (
                        <QuestionnaireForm
                            qform={questionnaire}
