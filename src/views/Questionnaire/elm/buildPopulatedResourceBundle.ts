...
function doSearch(
  ...
  callback: (n: FhirResource[] | null, e?: Error) => void
) {
  const q: Queries = {};
  let usePatient = true;
  ...
  switch (type) {
      ...
      if (type === 'PractitionerRole') {
        smart
          .request(`PractitionerRole?practitioner=${performer}`, {
            resolveReferences: ['location', 'organization'],
            flat: true,
            graph: false
          })
          .then(result => {
    ...
    case 'MedicationStatement':
      smart
        .request(`${type}?patient=${smart.patient.id}`, {
          resolveReferences: 'medicationReference',
          flat: true,
          graph: false
        })
        .then(result => {
          const finalResult: FhirResource[] = [];
  ...
  if (usePatient) {
    smart
      .request(`${type}?patient=${smart.patient.id}&${query}`)
